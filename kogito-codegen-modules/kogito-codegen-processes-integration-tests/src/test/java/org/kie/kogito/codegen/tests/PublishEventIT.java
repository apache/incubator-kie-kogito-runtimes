...
public class PublishEventIT extends AbstractCodegenIT {
    ...
    public void testServiceTaskProcessWithError() throws Exception {
        ...
        List<DataEvent<?>> events = publisher.extract().stream().filter(ProcessInstanceStateDataEvent.class::isInstance).collect(Collectors.toList());
        events.stream().forEach(System.out::println);
        assertThat(events).hasSize(1);

        ProcessInstanceStateEventBody body = assertProcessInstanceEvent(events.get(0), "ServiceProcessDifferentOperations", "Service Process", 5);
