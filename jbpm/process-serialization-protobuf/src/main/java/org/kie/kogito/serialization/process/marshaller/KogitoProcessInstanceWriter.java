...
public class KogitoProcessInstanceWriter {

    private MarshallerWriterContext context;

    public KogitoProcessInstanceWriter(MarshallerWriterContext context) {
    ...
    public void writeProcessInstance(WorkflowProcessInstanceImpl workFlow, OutputStream os) throws IOException {
        ...
        if (format != null && "json".equals(format)) {
            os.write(JsonFormat.printer().usingTypeRegistry(ProtobufTypeRegistryFactory.create()).print(piProtobuf).getBytes());
        } else {
