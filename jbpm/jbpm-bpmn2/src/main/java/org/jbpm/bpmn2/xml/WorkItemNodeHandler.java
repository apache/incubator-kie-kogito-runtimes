...
public class WorkItemNodeHandler extends AbstractNodeHandler {
    ...
    public void writeNode(Node node, StringBuilder xmlDump, int metaDataType) {
        ...
        if ("Service Task".equals(type)) {
            ...
            xmlDump.append("operationRef=\"" +
                    XmlBPMNProcessDumper.getUniqueNodeId(workItemNode) + "_ServiceOperation\" implementation=\"" + impl + "\" >"
                    + EOL);
            writeExtensionElements(workItemNode, xmlDump);
            xmlDump.append(
                    "      <ioSpecification>" + EOL +
                            "        <dataInput id=\"" + XmlBPMNProcessDumper.getUniqueNodeId(workItemNode)
                            + "_param\" name=\"Parameter\" />" + EOL +
                            "        <dataOutput id=\"" + XmlBPMNProcessDumper.getUniqueNodeId(workItemNode)
                            + "_result\" name=\"Result\" />" + EOL +
                            "        <inputSet>" + EOL +
                            "          <dataInputRefs>" + XmlBPMNProcessDumper.getUniqueNodeId(workItemNode)
                            + "_param</dataInputRefs>" + EOL +
                            "        </inputSet>" + EOL +
                            "        <outputSet>" + EOL +
                            "          <dataOutputRefs>" + XmlBPMNProcessDumper.getUniqueNodeId(workItemNode)
                            + "_result</dataOutputRefs>" + EOL +
                            "        </outputSet>" + EOL +
                            "      </ioSpecification>" + EOL);
            ...
            if (inMapping != null) {
                xmlDump.append(
                        "      <dataInputAssociation>" + EOL +
                                ...
                                "        <targetRef>" + XmlBPMNProcessDumper.getUniqueNodeId(workItemNode)
                                + "_param</targetRef>" + EOL +
                                "      </dataInputAssociation>" + EOL);
            }
            ...
            if (outMapping != null) {
                xmlDump.append(
                        "      <dataOutputAssociation>" + EOL +
                                "        <sourceRef>" + XmlBPMNProcessDumper.getUniqueNodeId(workItemNode)
                                + "_result</sourceRef>" + EOL +
    ...
    protected void writeIO(WorkItemNode workItemNode, StringBuilder xmlDump) {
        ...
        for (Map.Entry<String, String> entry : workItemNode.getInMappings().entrySet()) {
            xmlDump.append("          <dataInputRefs>" + XmlBPMNProcessDumper.getUniqueNodeId(workItemNode) + "_"
                    + XmlBPMNProcessDumper.replaceIllegalCharsAttribute(entry.getKey()) + "Input</dataInputRefs>" + EOL);
        }
        for (Map.Entry<String, Object> entry : workItemNode.getWork().getParameters().entrySet()) {
            if (entry.getValue() != null) {
                xmlDump.append("          <dataInputRefs>" + XmlBPMNProcessDumper.getUniqueNodeId(workItemNode) + "_"
        ...
        for (Map.Entry<String, String> entry : workItemNode.getInMappings().entrySet()) {
            ...
            xmlDump.append(
                    "        <sourceRef>" + XmlDumper.replaceIllegalChars(entry.getValue()) + "</sourceRef>" + EOL +
                            "        <targetRef>" + XmlBPMNProcessDumper.getUniqueNodeId(workItemNode) + "_"
            ...
            if (entry.getValue() != null) {
                ...
                xmlDump.append(
                        "        <targetRef>" + XmlBPMNProcessDumper.getUniqueNodeId(workItemNode) + "_"
                                ...
                                + XmlDumper.replaceIllegalChars(entry.getKey()) + "Input</to>" + EOL +
                                "        </assignment>" + EOL);
                xmlDump.append("      </dataInputAssociation>" + EOL);
            }
        }
    ...
    protected void writeOutputAssociation(WorkItemNode workItemNode, StringBuilder xmlDump) {
        for (Map.Entry<String, String> entry : workItemNode.getOutMappings().entrySet()) {
            ...
            xmlDump.append(
                    "        <sourceRef>" + XmlBPMNProcessDumper.getUniqueNodeId(workItemNode) + "_"
                            + XmlDumper.replaceIllegalChars(entry.getKey()) + "Output</sourceRef>" + EOL +
