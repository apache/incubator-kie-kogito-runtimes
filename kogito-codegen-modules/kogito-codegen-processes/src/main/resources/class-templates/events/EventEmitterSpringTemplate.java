...
public class SpringKafkaCloudEventEmitter<M> implements EventEmitter {
    ...
    public CompletionStage<Void> emit(DataEvent<?> event) {
        try {
            return emitter.send("$Trigger$", configBean.useCloudEvents() ? ceMarshaller.marshall(event.asCloudEvent(ceMarshaller.cloudEventDataFactory())) : marshaller.marshall(event.getData()))
                    .thenApply(r -> null);
        } catch (IOException e) {
