...
public class PredictionAwareHumanTaskLifeCycle extends BaseHumanTaskLifeCycle {
    ...
    public Map<String, Object> transitionTo(KogitoWorkItem workItem, WorkItemManager manager,
        ...
        if (targetPhase.id().equals(Active.ID)) {
            ...
            logger.debug("Prediction service returned confidence level {} for work item {}", outcome.getConfidenceLevel(),
                    humanTaskWorkItem.getStringId());

            if (outcome.isCertain()) {
                humanTaskWorkItem.getResults().putAll(outcome.getData());
                logger.debug("Prediction service is certain (confidence level {}) on the outputs, completing work item {}",
                        outcome.getConfidenceLevel(), humanTaskWorkItem.getStringId());
