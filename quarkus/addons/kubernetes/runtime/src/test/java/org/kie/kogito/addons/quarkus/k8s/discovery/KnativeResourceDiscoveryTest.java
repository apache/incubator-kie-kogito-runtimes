...
public class KnativeResourceDiscoveryTest {

    @KubernetesTestServer
    public KubernetesServer mockServer;

    @Inject
    KnativeResourceDiscovery knativeResourceDiscovery;
    ...
    public void testNotFoundKnativeService() {
        ...
        assertEquals(Optional.empty(),
                knativeResourceDiscovery.query(KnativeResourceUri.parse("serving.knative.dev/v1/Service/" + namespace + "/invalid")));
    }

    @Test
    public void testKnativeService() {
        var kubeURI = KnativeResourceUri.parse("serving.knative.dev/v1/Service/" + namespace + "/serverless-workflow-greeting-quarkus");
        ...
        Optional<String> url = knativeResourceDiscovery.query(kubeURI).map(URI::toString);
