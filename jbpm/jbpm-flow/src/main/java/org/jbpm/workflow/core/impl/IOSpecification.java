...
public class IOSpecification implements Serializable {
    ...
    private Assignment buildAssigment(Assignment assignment) {
        if (assignment.getFrom().hasExpression()) {
            ...
            String sourceExpression = assignment.getFrom().getExpression();
            sourceExpression = isConstant(sourceExpression) ? sourceExpression.substring(1, sourceExpression.length() -
                    1) : sourceExpression;
            assignment.setMetaData("Action", new StaticAssignment(sourceExpression, assignment.getTo()));
        }
        return assignment;
    }
