...
public class CallActivityHandler extends AbstractNodeHandler {
            ...
            if (subNode != null) {
                ...
                if (nl.getLength() > 1) {
                    ...
                    subProcessNode.addInAssociation(
                            new DataAssociation(subSubNode.getTextContent(), dataInputs.get(to),
                                    singletonList(new Assignment(null, subSubNode.getTextContent(), to)), null));
                    return;
                ...
                subProcessNode.addInAssociation(
                        new DataAssociation(result.toString(), dataInputs.get(to),
                                singletonList(new Assignment(null, result.toString(), to)), null));
            }
        }
    }
    ...
    protected void writeIO(SubProcessNode subProcessNode, StringBuilder xmlDump) {
        ...
        for (Map.Entry<String, String> entry : subProcessNode.getInMappings().entrySet()) {
            ...
            xmlDump.append(
                    "        <sourceRef>" + XmlDumper.replaceIllegalChars(entry.getValue()) + "</sourceRef>" + EOL +
                            "        <targetRef>" + XmlBPMNProcessDumper.getUniqueNodeId(subProcessNode) + "_"
        ...
        for (Map.Entry<String, String> entry : subProcessNode.getOutMappings().entrySet()) {
            ...
            xmlDump.append(
                    "        <sourceRef>" + XmlBPMNProcessDumper.getUniqueNodeId(subProcessNode) + "_"
