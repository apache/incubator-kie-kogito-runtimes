...
public class ServiceOperationsTest extends MockKubernetesServerSupport {
    ...
    private void assertDefaultServiceCreated(final String services)
            throws JsonParseException, JsonMappingException, IOException {
        this.assertDefaultServiceCreated(services, "127.0.0.1");
    }

    private void assertDefaultServiceCreated(final String services, final String assertIp)
            throws JsonParseException, JsonMappingException, IOException {
        ...
        assertThat(Integer.parseInt(new MapWalker(spec).mapToListMap("ports").listToMap(0).asMap().get("port").toString()),
                is(8080));
    }
    ...
    @Test
    public void whenThereIsAListOfServicesNamespacedLookingForLabelKey()
            throws JsonParseException, JsonMappingException, IOException {
        this.createMockService("service2", "192.168.0.1", Collections.singletonMap("service", null), MOCK_NAMESPACE);
        String services = this.getKubeClient().services()
                .listNamespaced(MOCK_NAMESPACE, Collections.singletonMap("service", null)).asJson();
        this.assertDefaultServiceCreated(services, "192.168.0.1");
    }
