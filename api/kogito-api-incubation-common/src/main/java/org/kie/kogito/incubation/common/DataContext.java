 ...
 * (or another DataContext) using the method <code>{@link Reshape}.from(object).to(type)</code></li>
 * <li>A DataContext can be always converted into another sub-type of DataContext.</li>
 ...
 * ctx.set("name", "Paul");
 * Person p = Convert.from(ctx).to(Person.class);
 * Person p = ctx.as(Person.class); // for internal DataContexts this is equivalent
