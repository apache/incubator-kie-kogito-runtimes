    ...
    protected Object evaluateExpression(Object expression, Map<String, Object> parameters) {
        try {
            ...
            if (expression instanceof CompiledScript) {
                logger.debug("About to evaluate compiled expression {} with bindings {} on engine", expression, parameters,
                        scriptEngine);
                result = ((CompiledScript) expression).eval(context);
            } else {
                logger.debug("About to evaluate expression {} with bindings {} on engine", expression, parameters,
                        scriptEngine);
