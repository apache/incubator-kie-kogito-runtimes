...
public class SubProcessHandler extends AbstractNodeHandler {
    ...
    @SuppressWarnings("unchecked")
	protected void handleCompositeContextNode( final JbpmNode node, final Element element, final String uri,
    ...
    @SuppressWarnings("unchecked")
	protected void handleForEachNode( final JbpmNode node, final Element element, final String uri,
    ...
    protected void applyAsync( JbpmNode node, boolean isAsync) {
        for (org.kie.api.definition.process.Node subNode: ((CompositeContextNode) node).getNodes()) {
            if (isAsync) {
                ...
                if (incoming != null) {
                    for (Connection con : incoming) {
                        if (con.getFrom() instanceof StartNode) {
                            (( JbpmNode )subNode).setMetaData("customAsync", Boolean.toString(isAsync));
                            return;
                        }
                    }
                }
