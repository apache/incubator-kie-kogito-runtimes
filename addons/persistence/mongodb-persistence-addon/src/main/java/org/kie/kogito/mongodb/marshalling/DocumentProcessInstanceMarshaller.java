...
public class DocumentProcessInstanceMarshaller {
    ...
    public ProcessInstanceDocument marshalProcessInstance(ProcessInstance<?> processInstance) {
        try {
            ...
            try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
                KogitoProcessMarshallerWriteContext context =
                        new KogitoProcessMarshallerWriteContext(baos, null, null, null, null, env);
                org.jbpm.marshalling.impl.ProcessInstanceMarshaller marshaller =
                        ProcessMarshallerRegistry.INSTANCE.getMarshaller(pi.getProcess().getType());
                JBPMMessages.ProcessInstance instance =
                        (JBPMMessages.ProcessInstance) marshaller.writeProcessInstance(context, pi);
    ...
    public WorkflowProcessInstance unmarshallWorkflowProcessInstance(ProcessInstanceDocument doc, Process<?> process) {
        ...
        try (ByteArrayInputStream bais = new ByteArrayInputStream(getDummyByteArray())) {
            ...
            org.jbpm.marshalling.impl.ProcessInstanceMarshaller marshaller =
                    ProcessMarshallerRegistry.INSTANCE.getMarshaller(instance.getProcessType());
    ...
    public <T> ProcessInstance<T> unmarshallProcessInstance(ProcessInstanceDocument doc, Process<?> process) {
        return (ProcessInstance<T>) ((AbstractProcess<?>) process)
                .createInstance(unmarshallWorkflowProcessInstance(doc, process));
    }

    @SuppressWarnings("unchecked")
    public <T> ProcessInstance<T> unmarshallReadOnlyProcessInstance(ProcessInstanceDocument doc, Process<?> process) {
        return (ProcessInstance<T>) ((AbstractProcess<?>) process)
                .createReadOnlyInstance(unmarshallWorkflowProcessInstance(doc, process));
    }
