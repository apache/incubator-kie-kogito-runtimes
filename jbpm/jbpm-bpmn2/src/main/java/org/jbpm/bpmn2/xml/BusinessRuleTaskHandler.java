...
public class BusinessRuleTaskHandler extends AbstractNodeHandler {
    ...
    public void writeNode(Node node, StringBuilder xmlDump, int metaDataType) {
        ...
        if (ruleType != null) {
            xmlDump.append(
                    "g:ruleFlowGroup=\"" + XmlBPMNProcessDumper.replaceIllegalCharsAttribute(ruleType.getName()) + "\" " + EOL);
            // else DMN
        }

        xmlDump.append(" implementation=\"" + XmlBPMNProcessDumper.replaceIllegalCharsAttribute(ruleSetNode.getLanguage())
                + "\" >" + EOL);
    ...
    protected void readDataOutputAssociation(org.w3c.dom.Node xmlNode, RuleSetNode ruleSetNode,
        ...
        ruleSetNode.addOutAssociation(
                new DataAssociation(sources.stream().map(source -> dataOutputs.get(source)).collect(Collectors.toList()),
                        target, assignments, transformation));
    }
    ...
    protected void writeIO(RuleSetNode ruleSetNode, StringBuilder xmlDump) {
        ...
        for (Map.Entry<String, String> entry : ruleSetNode.getInMappings().entrySet()) {
            ...
            xmlDump.append(
                    "        <sourceRef>" + XmlDumper.replaceIllegalChars(entry.getValue()) + "</sourceRef>" + EOL +
                            "        <targetRef>" + XmlBPMNProcessDumper.getUniqueNodeId(ruleSetNode) + "_"
        ...
        for (Map.Entry<String, String> entry : ruleSetNode.getOutMappings().entrySet()) {
            ...
            xmlDump.append(
                    "        <sourceRef>" + XmlBPMNProcessDumper.getUniqueNodeId(ruleSetNode) + "_"
