 ...
 * participates in context restoration by restoring OTel keys after core context is set.
 *
 * Thread Safety: This class is thread-safe using MDC's inherent thread-local isolation.
 */
@ApplicationScoped
public class OtelContextExtension implements ContextExtension {
    ...
    @PostConstruct
    void register() {
        ProcessInstanceContext.registerExtension(MDC_KEY_PREFIX, this);
    ...
    public void restoreKeys(Map<String, String> preservedKeys) {
        for (Map.Entry<String, String> entry : preservedKeys.entrySet()) {
            MDC.put(entry.getKey(), entry.getValue());
        }

        if (LOGGER.isTraceEnabled()) {
            LOGGER.trace("Restored {} OTel MDC keys after context restoration", preservedKeys.size());
        }
    }
}