...
public class NodeOtelEventListenerTest {
    ...
    @Mock
    private SonataFlowOtelConfig.ProcessEventConfig processEventConfig;
    ...
    public void shouldCreateNodeSpanOnBeforeNodeTriggered() {
        // Set up config mocking for this test
        when(config.events()).thenReturn(eventConfig);
        when(eventConfig.enabled()).thenReturn(true);
        when(eventConfig.processEvents()).thenReturn(processEventConfig);
        when(processEventConfig.enabled()).thenReturn(true);
        when(processEventConfig.includeStart()).thenReturn(true);
