...
public class ForEachNodeHandler extends AbstractCompositeNodeHandler {
    ...
    public void writeNode(Node node, StringBuilder xmlDump, int metaDataType) {
        ...
        if (parameterName != null) {
            xmlDump.append(
                    "        <dataInput id=\"" + XmlBPMNProcessDumper.getUniqueNodeId(forEachNode)
        ...
        if (collectionExpression != null) {
            xmlDump.append(
                    "      <dataInputAssociation>" + EOL +
                            ...
                            "        <targetRef>" + XmlBPMNProcessDumper.getUniqueNodeId(forEachNode) + "_input</targetRef>"
                            + EOL +
                            "      </dataInputAssociation>" + EOL);
        }
        ...
        xmlDump.append(
                "      <multiInstanceLoopCharacteristics>" + EOL +
                        "        <loopDataInputRef>" + XmlBPMNProcessDumper.getUniqueNodeId(forEachNode)
                        + "_input</loopDataInputRef>" + EOL);
        if (parameterName != null) {
            xmlDump.append("        <inputDataItem id=\"" + XmlBPMNProcessDumper.replaceIllegalCharsAttribute(parameterName)
                    + "\" itemSubjectRef=\"" + XmlBPMNProcessDumper.getUniqueNodeId(forEachNode) + "_multiInstanceItemType\"/>"
                    + EOL);
        }
