...
class EvaluateEventTypeTest {
    ...
    private static final Map<EvaluateEventType, Pair<String, Class<?>>> CHECK_MAP =
            new HashMap<EvaluateEventType, Pair<String, Class<?>>>() {
                {
                    ...
                    put(EvaluateEventType.BEFORE_EVALUATE_CONTEXT_ENTRY,
                            new Pair<>("beforeEvaluateContextEntry", BeforeEvaluateContextEntryEvent.class));
                    put(EvaluateEventType.AFTER_EVALUATE_CONTEXT_ENTRY,
                            new Pair<>("afterEvaluateContextEntry", AfterEvaluateContextEntryEvent.class));
                    put(EvaluateEventType.BEFORE_EVALUATE_DECISION,
                            new Pair<>("beforeEvaluateDecision", BeforeEvaluateDecisionEvent.class));
                    put(EvaluateEventType.AFTER_EVALUATE_DECISION,
                            new Pair<>("afterEvaluateDecision", AfterEvaluateDecisionEvent.class));
                    put(EvaluateEventType.BEFORE_EVALUATE_DECISION_SERVICE,
                            new Pair<>("beforeEvaluateDecisionService", BeforeEvaluateDecisionServiceEvent.class));
                    put(EvaluateEventType.AFTER_EVALUATE_DECISION_SERVICE,
                            new Pair<>("afterEvaluateDecisionService", AfterEvaluateDecisionServiceEvent.class));
                    put(EvaluateEventType.BEFORE_EVALUATE_DECISION_TABLE,
                            new Pair<>("beforeEvaluateDecisionTable", BeforeEvaluateDecisionTableEvent.class));
                    put(EvaluateEventType.AFTER_EVALUATE_DECISION_TABLE,
                            new Pair<>("afterEvaluateDecisionTable", AfterEvaluateDecisionTableEvent.class));
    ...
    void testExistingEvents() {
        CHECK_MAP.forEach((type, checkPair) -> assertDoesNotThrow(
                ...
                () -> String.format("Listener method \"%s(%s)\" not found for EvaluateEventType.%s", checkPair.getLeft(),
                        checkPair.getRight().getSimpleName(), type)));
    }
