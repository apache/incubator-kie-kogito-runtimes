 ...
 * wrapped {@link WorkItemHandler} instance throws an exception, the {@link SignallingTaskHandlerDecorator} instance must either
 * be
 * reset via the {@link SignallingTaskHandlerDecorator#clear()} or
 * {@link SignallingTaskHandlerDecorator#clearProcessInstance(Long)}
 * methods.
 * <p>
...
public class SignallingTaskHandlerDecorator extends AbstractExceptionHandlingTaskHandler {
    ...
    /**
     * Constructs an instance that uses the given <code>eventType</code> parameter to signal the process instance using the
     * given
     * {@link KieSession} <code>ksession</code> parameter when an instance of the class specified by the
     * <code>originalTaskHandlerClass</code> throws an exception upon
    ...
    @Override
    public void handleExecuteException(Throwable cause, org.kie.kogito.internal.process.runtime.KogitoWorkItem workItem,
            org.kie.kogito.internal.process.runtime.KogitoWorkItemManager manager) {
        if (getAndIncreaseExceptionCount(workItem.getProcessInstanceStringId()) < exceptionCountLimit) {
            ...
            ((KogitoWorkItemManager) manager).signalEvent(this.eventType, (KogitoWorkItem) workItem,
                    workItem.getProcessInstanceStringId());
        } else {
            if (cause instanceof RuntimeException) {
                throw (RuntimeException) cause;
            } else {
                throw new WorkItemHandlerRuntimeException(cause,
                        "Signalling process instance " + workItem.getProcessInstanceId() + " with '" + this.eventType
    ...
    @Override
    public void handleAbortException(Throwable cause, org.kie.kogito.internal.process.runtime.KogitoWorkItem workItem,
            org.kie.kogito.internal.process.runtime.KogitoWorkItemManager manager) {
        if (getAndIncreaseExceptionCount(workItem.getProcessInstanceStringId()) < exceptionCountLimit) {
            workItem.getParameters().put(this.workItemExceptionParameterName, cause);
            ((KogitoWorkItemManager) manager).signalEvent(this.eventType, (KogitoWorkItem) workItem,
