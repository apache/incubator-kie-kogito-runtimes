...
public class MultipleProcessInstanceDataEventDeserializer extends JsonDeserializer<MultipleProcessInstanceDataEvent> implements ResolvableDeserializer {
    ...
    public static Collection<ProcessInstanceDataEvent<? extends KogitoMarshallEventSupport>> readFromBytes(byte[] binaryValue) throws IOException {
        try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(binaryValue))) {
    ...
    private static <T extends ProcessInstanceDataEvent<V>, V extends KogitoMarshallEventSupport & CloudEventVisitor> T buildDataEvent(DataInput in, T instance, V body,
            ProcessInstanceDataEventExtensionRecord info) throws IOException {
        T cloudEvent = KogitoDataEventSerializationHelper.readCloudEventAttrs(in, instance);
        populateCloudEvent(cloudEvent, info);
        body.readEvent(in);
    ...
    private static void populateCloudEvent(ProcessInstanceDataEvent<?> event, ProcessInstanceDataEventExtensionRecord info) {
