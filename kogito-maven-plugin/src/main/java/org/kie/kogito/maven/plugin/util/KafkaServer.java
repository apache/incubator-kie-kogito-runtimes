...
public class KafkaServer {
    ...
    private int port = -1;
    private kafka.server.KafkaServer server;
     ...
     */
    public KafkaServer(Supplier<String> zookeeperConnection, int port) {
    ...
    public KafkaServer setProperties(Properties properties) {
        ...
        properties.stringPropertyNames().forEach(propName -> setProperty(propName, properties.getProperty(propName)));
        return this;
    }
    ...
    public synchronized KafkaServer startup() {
        ...
        Properties runningConfig = config();
        runningConfig.setProperty(KafkaConfig.LogDirProp(), logsDir.getAbsolutePath());
        runningConfig.setProperty(KafkaConfig.OffsetsTopicReplicationFactorProp(), String.valueOf(1));

        // Determine the port and adjust the configuration ...
        runningConfig.setProperty(KafkaConfig.PortProp(), Integer.toString(port));

        // Start the server ...
        try {
            LOGGER.debug("Starting Kafka broker {} at {} with storage in {}", brokerId, getConnection(), logsDir.getAbsolutePath());
            server = new kafka.server.KafkaServer(new KafkaConfig(runningConfig), Time.SYSTEM, scala.Option.apply(null),
    ...
    public synchronized void deleteData() {
        if (server == null) {
            // Delete all data ...
            try {
                Files.deleteIfExists(this.logsDir.toPath());
            } catch (IOException e) {
