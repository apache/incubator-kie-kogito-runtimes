...
import org.kie.kogito.serialization.protobuf.ProtobufTypeRegistryFactory;
...
public class KogitoProcessInstanceReader {
    ...
    public KogitoProcessInstanceReader(MarshallerReaderContext context) {
    ...
    public RuleFlowProcessInstance read(InputStream input) throws IOException {
        ...
        if (format != null && "json".equals(format)) {
            ...
            JsonFormat.parser().usingTypeRegistry(ProtobufTypeRegistryFactory.create()).ignoringUnknownFields().merge(new InputStreamReader(input), builder);
    ...
    private RuleFlowProcessInstance buildWorkflow(KogitoProcessInstanceProtobuf.ProcessInstance processInstanceProtobuf) {
        ...
        AbstractProcess<?> process = context.get(MarshallerContextName.MARSHALLER_PROCESS);
        ...
        try {
            ...
            if (nodeInstance instanceof HumanTaskNodeInstance) {
                ...
                if (workItemDataMessage.is(HumanTaskWorkItemData.class)) {
                    ...
                    workItem.getComments().putAll(workItemData.getCommentsList().stream().map(this::buildComment).collect(Collectors.toMap(Comment::getId, identity())));
                    workItem.getAttachments().putAll(workItemData.getAttachmentsList().stream().map(this::buildAttachment).collect(Collectors.toMap(Attachment::getId, identity())));
