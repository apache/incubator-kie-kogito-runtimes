...
import org.kie.kogito.serialization.protobuf.KogitoNodeInstanceContentsProtobuf.RuleSetNodeInstanceContent.TextMapEntry;
...
import org.kie.kogito.serialization.protobuf.KogitoTypesProtobuf.SLAContext;
import org.kie.kogito.serialization.protobuf.KogitoTypesProtobuf.WorkflowContext;
...
public class ProcessInstanceReader {
    ...
    private static String PROCESS_INSTANCE_ENV = "PROCESS_INSTANCE_KEY";
    private Environment env;

    public ProcessInstanceReader(Environment environment) {
    ...
    public RuleFlowProcessInstance read(InputStream input) throws IOException {
        ...
        env.set(PROCESS_INSTANCE_ENV, null);
    ...
    private RuleFlowProcessInstance buildWorkflow(KogitoProcessInstanceProtobuf.ProcessInstance processInstanceProtobuf) {
        ...
        env.set(PROCESS_INSTANCE_ENV, processInstance);
        ...
        for (KogitoTypesProtobuf.NodeInstance nodeInstanceProtobuf : workflowContext.getNodeInstanceList()) {
            ...
            processInstance.addNodeInstance(nodeInstanceImpl);
        }

        for (KogitoTypesProtobuf.NodeInstanceGroup group : workflowContext.getExclusiveGroupList()) {
            Function<String, KogitoNodeInstance> finder = nodeInstanceId -> ((org.jbpm.workflow.instance.NodeInstanceContainer) processInstance).getNodeInstance(nodeInstanceId, true);
        ...
        if (workflowContext.getVariableCount() > 0) {
            ...
            buildVariables(workflowContext.getVariableList()).forEach(v -> variableScopeInstance.internalSetVariable(v.getName(), v));
        }
    ...
    protected NodeInstanceImpl buildNodeInstance(com.google.protobuf.Any nodeContentProtobuf) {
        try {
            if (nodeContentProtobuf.is(RuleSetNodeInstanceContent.class)) {
                ...
                EventNodeInstanceContent content = nodeContentProtobuf.unpack(EventNodeInstanceContent.class);
                return buildEventNodeInstance(content);
    ...
    private NodeInstanceImpl buildEventNodeInstance(EventNodeInstanceContent content) {
        EventNodeInstance nodeInstance = new EventNodeInstance();
    ...
    private NodeInstanceImpl buildRuleSetNodeInstance(RuleSetNodeInstanceContent content) {
        ...
        ((RuleSetNodeInstance) nodeInstance).setRuleFlowGroup(content.getRuleFlowGroup());
        if (content.getTimerInstanceIdCount() > 0) {
            List<String> timerInstances = new ArrayList<>();
            for (String _timerId : content.getTimerInstanceIdList()) {
                timerInstances.add(_timerId);
        ...
        if (content.getMapEntryCount() > 0) {
            Map<String, FactHandle> factInfo = new HashMap<String, FactHandle>();

            for (TextMapEntry entry : content.getMapEntryList()) {
        ...
        try {
            ...
            if (nodeInstance instanceof HumanTaskNodeInstance) {
                ...
                if (workItemDataMessage.is(HumanTaskWorkItemData.class)) {
                    ...
                    workItem.getComments().putAll(workItemData.getCommentsList().stream().map(this::convert).collect(Collectors.toMap(k -> k.getId(), k -> k)));
                    workItem.getAttachments().putAll(workItemData.getAttachmentsList().stream().map(this::convert).collect(Collectors.toMap(k -> k.getId(), k -> k)));

                }
            }

            RuleFlowProcessInstance processInstance = (RuleFlowProcessInstance) this.env.get(PROCESS_INSTANCE_ENV);
            ...
            workItem.setDeploymentId(processInstance.getDeploymentId());
            ...
            buildVariables(content.getVariableList()).forEach(var -> nodeInstance.getWorkItem().setParameter(var.getName(), var.getValue()));

            return nodeInstance;
        } catch (InvalidProtocolBufferException ex) {
            throw new RuntimeException("cannot unpack node instance", ex);
        }
    }
    ...
    private WorkItemNodeInstance instanceWorkItem(WorkItemNodeInstanceContent content) {
        if (content.hasWorkItemData()) {
            ...
            if (workItemDataMessage.is(HumanTaskWorkItemData.class)) {
                ...
                throw new RuntimeException("Don't know which type of work item is");
            }
        } else {
    ...
    private void buildWorkflowContext(CompositeContextNodeInstance container, WorkflowContext workflowContext) {
        if (workflowContext.getNodeInstanceCount() > 0) {
            for (KogitoTypesProtobuf.NodeInstance nodeInstanceProtobuf : workflowContext.getNodeInstanceList()) {
                ...
                container.addNodeInstance(nodeInstanceImpl);
            }
        }
        for (KogitoTypesProtobuf.NodeInstanceGroup group : workflowContext.getExclusiveGroupList()) {
    ...
    private List<Variable> buildVariables(List<KogitoTypesProtobuf.Variable> variablesProtobuf) {
        ...
        for (KogitoTypesProtobuf.Variable var : variablesProtobuf) {
            ...
            ByteString storedValue = var.getValue();
            if (!storedValue.isEmpty()) {
                try (InputStream is = new ByteArrayInputStream(storedValue.toByteArray()); ObjectInputStream ois = new ObjectInputStream(is);) {
                    storedVar.setValue(ois.readObject());
    ...
    private Comment convert(KogitoWorkItemsProtobuf.Comment comment) {
    ...
    private Attachment convert(KogitoWorkItemsProtobuf.Attachment attachment) {
