...
public class EventNodeBuilder implements ProcessNodeBuilder {
    ...
    public void build(Process process, ProcessDescr processDescr, ProcessBuildContext context, Node node) {
        ...
        // for boundary event we need to compile the transformations
        if (node instanceof BoundaryEventNode) {
            BoundaryEventNode boundaryEventNode = (BoundaryEventNode) node;
            boundaryEventNode.getOutAssociations().stream()
                                                  .filter(e -> e.getTransformation()!= null)
                                                  .map(e -> e.getTransformation())
                                                  .forEach(dataAssociationTransformation -> {
                WorkflowProcess wfProcess = (WorkflowProcess) process;
                Map<String, Object> parameters = new HashMap<String, Object>();
                parameters.put("imports", wfProcess.getImports());
                parameters.put("classloader", context.getConfiguration().getClassLoader());
