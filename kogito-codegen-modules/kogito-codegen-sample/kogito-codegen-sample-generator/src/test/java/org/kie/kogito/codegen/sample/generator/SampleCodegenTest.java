...
class SampleCodegenTest {
    ...
    void section() {
        SampleCodegen sampleCodegen =
                SampleCodegen.ofCollectedResources(JavaKogitoBuildContext.builder().build(), Collections.emptyList());
    ...
    void generate(KogitoBuildContext.Builder contextBuilder) {
        ...
        List<GeneratedFile> generatedRests =
                generatedFiles.stream().filter(gf -> gf.type() == REST_TYPE).collect(Collectors.toList());
        ...
        Optional<FieldDeclaration> optionalFieldDeclaration =
                compilationUnit.findFirst(FieldDeclaration.class, SampleCodegen::isSampleRuntimeField);
