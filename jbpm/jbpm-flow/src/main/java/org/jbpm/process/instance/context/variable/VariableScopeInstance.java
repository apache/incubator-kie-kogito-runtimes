...
public class VariableScopeInstance extends AbstractContextInstance {
    ...
    public void setVariable(KogitoNodeInstance nodeInstance, String name, Object value) {
        ...
        if (oldValue != null && getVariableScope().isReadOnly(name)) {
            throw new VariableViolationException(getProcessInstance().getStringId(), name,
                    "Variable '" + name + "' is already set and is marked as read only");
        }
        KogitoProcessEventSupport processEventSupport =
                (KogitoProcessEventSupport) ((InternalProcessRuntime) getProcessInstance()
    ...
    public void enforceRequiredVariables() {
        ...
        for (Variable variable : variableScope.getVariables()) {
            if (variableScope.isRequired(variable.getName()) && !variables.containsKey(variable.getName())) {
                throw new VariableViolationException(getProcessInstance().getStringId(), variable.getName(),
